...
void example()
{
    ...
    for(int i = 0; i < 2 * N_MAX; ++ i)
    {
        ...
        {
            // generate pseudodata sample
            data->Fill(rand_gaus_1, A_coeff);
            data->Fill(rand_gaus_2, B_coeff);
            data->Fill(rand_exp_1, C_coeff);
        }
    }

    
    // set pseudodata bin errors manually
    for(Int_t i = 1; i <= mc_1->GetNbinsX(); ++ i)
    {
        double c = data->GetBinContent(i);
    ...
    //mc_3->Draw("histsame");
    c_all_before->Show();
    c_all_before->SaveAs("c_all_before_example3.png");
    ...
    //mc_3->Draw("histsame");
    c_all_after->Show();
    c_all_after->SaveAs("c_all_after_example3.png");
